...
class MakeBevImages():
    ...
    def save_dataset(self, ego_pose_msg, local_map_msg, velo_msg, front_img_msg, fl_img_msg, fr_img_msg, trk_bboxes_msg, can_msg):
        ...
        ego_yaw = euler[2]
        data = "%f %f %f %f %f %f\n"%(velo_msg.header.stamp.to_sec(), velo_msg.header.stamp.to_nsec(), ego_pose_msg.pose.position.x, ego_pose_msg.pose.position.y, ego_pose_msg.pose.position.z, ego_yaw)
        f.write(data)
        f.close()

...
#define VPointCloud pcl::PointCloud<VPoint>
...
private:
    ...
    ros::Publisher cluster_label_pub_;
...
ScanLineRun::ScanLineRun():node_handle_("~"){
    ...
    node_handle_.param("sensor_model", sensor_model_, 64); //32 // 16
    ...
    laser_row_ = std::vector<SLRPointXYZIRL>(4000, p_dummy); // 2251 // 1800
    laser_frame_ = std::vector<std::vector<SLRPointXYZIRL> >(64, laser_row_); // 32 // 16
    ...
    node_handle_.param("th_run", th_run_, 0.15); // 0.15
    ROS_INFO("Point-to-Run Threshold: %f", th_run_);

    node_handle_.param("th_merge", th_merge_, 1.5); // 0.5 //1.5
    ...
    cluster_points_pub_ = node_handle_.advertise<sensor_msgs::PointCloud2 >(cluster_topic, 10);
    cluster_label_pub_ = node_handle_.advertise<VPointCloud>("/cluster_label", 10);
...
/*
    ...
    @param ns: The name string.The name string.
...
visualization_msgs::Marker mark_cluster(pcl::PointCloud<SLRPointXYZIRL>::Ptr cloud_cluster, 
  ...
  //marker.header.frame_id = "/horizontal_laser_link"; 
...
void ScanLineRun::velodyne_callback_(const sensor_msgs::PointCloud2ConstPtr& in_cloud_msg){
    ...
    SLRPointXYZIRL p_dummy;
    p_dummy.intensity = -1;
    laser_row_ = std::vector<SLRPointXYZIRL> (4000, p_dummy); // 2251
    laser_frame_ = std::vector<std::vector<SLRPointXYZIRL> >(64, laser_row_); // 32
    ...
    if(laserCloud->points.size()>0){
        ...
        //cluster_msg.header.frame_id = "/horizontal_laser_link";
        cluster_msg.header.frame_id = laserCloudIn.header.frame_id;
        cluster_points_pub_.publish(cluster_msg);

        VPointCloud label_cloud;
        label_cloud.header.frame_id = cluster_msg.header.frame_id;
        double rot_ang = -M_PI*11./180.;
        for (int i = 0;i<laserCloud->points.size();i++)
        {
            VPoint tmp;
            double x = laserCloud->points[i].x;
            double y = laserCloud->points[i].y;
            double z = laserCloud->points[i].z;

            double x_rot = x;
            double z_rot = z;
            // double x_rot = cos(rot_ang)*x + sin(rot_ang)*z;
            // double z_rot = -sin(rot_ang)*x + cos(rot_ang)*z;
            tmp.x = x_rot;
            tmp.y = y;
            tmp.z = z_rot;
            tmp.ring = laserCloud->points[i].label;
            tmp.intensity = laserCloud->points[i].intensity;
            label_cloud.push_back(tmp);
        }
        cluster_label_pub_.publish(label_cloud);
        ...
        for (int j = 0;j<cnt;j++)
        {
            pcl::PointCloud<SLRPointXYZIRL>::Ptr cluster_label_tmp(new pcl::PointCloud<SLRPointXYZIRL>());
            cluster_label_tmp->header.frame_id = cluster_msg.header.frame_id;
            for (int i = 0;i<laserCloud->points.size();i++)
            {
                if (laserCloud->points[i].label == j)
                {
                    SLRPointXYZIRL tmp;
                    tmp.x = laserCloud->points[i].x;
                    tmp.y = laserCloud->points[i].y;
                    tmp.z = laserCloud->points[i].z;
                    tmp.intensity = laserCloud->points[i].intensity;
                    tmp.ring = laserCloud->points[i].ring;
                    tmp.label = laserCloud->points[i].label;
                    cluster_label_tmp->push_back(tmp);

                }
            }
            visualization_msgs::Marker marker= mark_cluster(cluster_label_tmp, "label", j, 255, 255, 255);
            markerArr.markers.push_back(marker);
        }
        marker_array_pub_.publish(markerArr);
    //     visualization_msgs::Marker mark_cluster(pcl::PointCloud<SLRPointXYZIRL>::Ptr cloud_cluster, 
    // std::string ns ,int id, float r, float g, float b)


        // VPointCloud tmp_cloud;
        // tmp_cloud.header.frame_id = "/velodyne";
        
        // for (int i = 0; i< laserCloud->points.size();i++)
        // {
        //     if (laserCloud->points[i].label == 1)
        //     {
        //         VPoint tmp;
        //         tmp.x = laserCloud->points[i].x;
        //         tmp.y = laserCloud->points[i].y;
        //         tmp.z = laserCloud->points[i].z;
        //         tmp.intensity = laserCloud->points[i].intensity;
        //         tmp_cloud.push_back(tmp);
        //     }
        // }
                       

        // cluster_label_pub_.publish(tmp_cloud);
    }
#ifdef IO
